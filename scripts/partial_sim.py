...
if __name__=='__main__':
    ...
    dsv = dsv.sel(x=slice(xmin, xmax), y=slice(ymax, ymin))
    ...
    np.save(Path('../results/partial_nonstationary_sim.npy'), sim)
    ...
    im = plt.pcolormesh(xx/1000, yy/1000, sim+trend, cmap=cm.batlowW)
    plt.axis('scaled')
    plt.xlabel('X [km]')
    plt.ylabel('Y [km]')
    ...
    plt.savefig(Path('../figures/partial_simulation_nonstationary.png'), dpi=300, bbox_inches='tight')
    plt.show()